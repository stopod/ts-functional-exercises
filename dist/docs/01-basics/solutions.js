...
// より実用的な解答（スペース区切り）
const sentenceWithSpaces = words.reduce((acc, word, index) => index === 0 ? word : acc + ' ' + word, '');
 ...
 */
const createDivisibilityChecker = (divisor) => (num) => num % divisor === 0;
// 使用例
 ...
 */
const createWrapper = (wrapChar) => (text) => {
...
// 使用例
const filterEvenNumbers = createFilter((n) => n % 2 === 0);
const filterLongStrings = createFilter((s) => s.length > 5);
