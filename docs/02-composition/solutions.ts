...
// 使用例 - 完全な型推論デモ
const pipeResult1 = pipe('  Hello World  ', 
  (s: string) => s.trim(),
  (s: string) => s.toUpperCase(),
  (s: string) => s + '!'
); // 型: string

const pipeResult2 = pipe(5,
  (n: number) => n + 1,
  (n: number) => n * 2,
  (n: number) => n.toString(),
...
const condPipe = <T>(
  ...
  truePipe: (value: T) => T,
  falsePipe: (value: T) => T
) => (value: T): T => condition(value) ? truePipe(value) : falsePipe(value);
...
): T[] => array.map(item => condition(item) ? trueMapper(item) : falseMapper(item));
...
type ProductWithParsedPriceAndStock = Omit<ProductWithParsedPrice, 'stock'> & { stock: number };

const parseStock = (product: ProductWithParsedPrice): ProductWithParsedPriceAndStock => ({
  ...product,
  stock: parseInt(product.stock, 10)
});

// 3. 在庫有無フラグを追加 - 完全な型安全実装
type ProductWithAvailability = ProductWithParsedPriceAndStock & { isAvailable: boolean };

const addAvailabilityFlag = (product: ProductWithParsedPriceAndStock): ProductWithAvailability => ({
  ...product,
  isAvailable: product.stock > 0
});

// 4. 価格表示用文字列を作成 - 完全な型安全実装
const addPriceDisplay = (product: ProductWithAvailability): ProcessedProduct => ({
  id: product.id,
  name: product.name,
  price: product.price,
